...
use wdic_gateway::{
    Gateway, GatewayConfig, PerformanceMonitor, CompressionManager, 
    Registry, RegistryEntry, UdpBroadcastManager
};
    ...
    for i in 0..20 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let run_handle = tokio::spawn(async move {
                timeout(Duration::from_millis(100), gateway.run()).await
            });
...
async fn test_registry_stress() -> Result<()> {
    ...
    println!("- 操作吞吐量: {:.2} ops/sec", 1000.0 / duration.as_secs_f64());
...
async fn test_compression_stress() -> Result<()> {
    ...
    println!("- 压缩吞吐量: {:.0} 操作/秒", total_compress as f64 / duration.as_secs_f64());
...
async fn test_performance_monitor_stress() -> Result<()> {
    ...
    println!("- 操作吞吐量: {:.2} 监控器/秒", 50.0 / duration.as_secs_f64());
...
async fn test_memory_stress() -> Result<()> {
    ...
    println!("初始内存使用: {:.2} MB", initial_memory.current_usage as f64 / 1024.0 / 1024.0);
    ...
    for i in 0..100 {
        ...
        if i % 10 == 0 {
            ...
            println!("内存使用 ({}): {:.2} MB", 
                     i, current_memory.current_usage as f64 / 1024.0 / 1024.0);
        }
    }
    
    let peak_memory = monitor.get_memory_metrics().await;
    println!("峰值内存使用: {:.2} MB", peak_memory.current_usage as f64 / 1024.0 / 1024.0);
    ...
    println!("最终内存使用: {:.2} MB", final_memory.current_usage as f64 / 1024.0 / 1024.0);
