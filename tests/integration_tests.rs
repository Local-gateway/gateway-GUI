...
use wdic_gateway::{Gateway, GatewayConfig, UdpBroadcastManager, Registry, PerformanceMonitor};
use anyhow::Result;
...
async fn test_p2p_discovery() -> Result<()> {
    ...
    let handle1 = tokio::spawn(async move {
        timeout(Duration::from_millis(500), gateway1.run()).await
    });
    
    let handle2 = tokio::spawn(async move {
        timeout(Duration::from_millis(500), gateway2.run()).await
    });
...
async fn test_compression_system() -> Result<()> {
    ...
    assert!(stats.compress_count.load(std::sync::atomic::Ordering::Relaxed) > 0);
    assert!(stats.decompress_count.load(std::sync::atomic::Ordering::Relaxed) > 0);
...
async fn test_performance_benchmarks() -> Result<()> {
    ...
    // 运行延迟基准测试
    let latency_result = monitor.run_latency_benchmark("integration_test", 100).await?;
    assert!(latency_result.average_latency > 0.0);
    assert!(latency_result.operations > 0);
    
    // 运行吞吐量基准测试
    let throughput_result = monitor.run_throughput_benchmark("integration_throughput", &test_suite).await?;
